...
then
    edgelist_filename=../HEADNET/datasets/${dataset}/edgelist.tsv.gz
else 
    edgelist_filename=$(printf ../HEADNET/edgelists/${dataset}/seed=%03d/training_edges/edgelist.tsv.gz ${seed})
fi

embedding_directory=$(printf embeddings/directed/${dataset}/dim=%02d/seed=%03d/${exp} ${dim} ${seed})
